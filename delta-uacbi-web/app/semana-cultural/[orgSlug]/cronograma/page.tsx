...
import { getActiveEvents, resolveOrganization, getActiveEdition } from "@/lib/semana-cultural";
...
function getEventStyle(event: EventItem) {
  ...
  return fallbackScoredStyle;
}

function getEventInlineStyle(event: EventItem): { borderColor: string; backgroundColor: string; color: string } | undefined {
  const hex = event.scoreCategory?.colorHex;
  if (!event.isScored || !hex) return undefined;
  return { borderColor: hex, backgroundColor: `${hex}26`, color: hex };
}
    ...
    <>
      ...
      <main className="container py-10">
        ...
        <p className="mt-2 text-muted-foreground">
          Actividades programadas para {edition?.name ?? "la Semana Cultural"}.
        </p>
