...
function getScoredEventLabel(event: { name: string; scoreCategory: { label: string } | null }) {
  return `${event.name} · ${event.scoreCategory?.label ?? "Sin categoría"}`;
}
...
export default async function AdminPuntosPage({
  ...
  const selectedEventId = queryParams.eventId || "";
  const selectedUnidad = queryParams.unidad || "";
  ...
  const registeredTeams = selectedEvent
    ? Array.from(
        ...
        .filter((team) =>
          selectedUnidad
            ? getTeamComposition(team.members.map((member) => member.academicUnitCode)) === selectedUnidad
            : true
        )
    ...
    new Set(
      events.flatMap((event) =>
        event.registrations.map((registration) =>
          getTeamComposition(registration.team.members.map((member) => member.academicUnitCode))
        )
      )
    )
  ).sort();
      ...
      <main className="container py-10">
        ...
        <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <section className="card-next rounded-3xl p-6">
            ...
            <form action={assignScore.bind(null, organization.id, orgSlug)} className="mt-6 grid gap-4">
              <div>
                    ...
                    logs.map((log) => (
                      <tr key={log.id} className="border-t border-white/10">
                        ...
                        <td className="px-4 py-3">
                          <form action={deleteScoreLog.bind(null, organization.id, orgSlug, log.id, log.teamId)}>
                            <button
                              type="submit"
